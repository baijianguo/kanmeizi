 ...
 * 
 * @author launliu robinsli copy it from ui to corejava,to support sdk
 * 
 */
public class X5WebViewScrollbarDrawable extends Drawable {
	...
	public X5WebViewScrollbarDrawable(boolean mVertical, Drawable mDrawable,
			int margin, int size, int alpha) {
	 ...
	 * Drawable.setBounds调用X5WebViewScrollbarDrawable
	 * .onBoundsChanged来设置此次滚动条显示的位置.
	 * 但是bug[49197521]，webview的滚动条在设置完显示位置后draw之前，中间显示了下拉列表框。
	 * 共享图片的位置被设置成下拉列表框的位置而没清理。 导致页面滚动条用下拉列表框的位置进行显示.
	 * 
	 * 把位置设置放到draw的时候
	 */
	@Override
