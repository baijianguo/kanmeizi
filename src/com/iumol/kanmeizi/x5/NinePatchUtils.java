...
/***
 * 
 * robins copy this file from ui project to core 2014-02 for sdk will read
 * scrollbar picture and listbox will read backgroud picture. 2014.2
 * getDrawableFromDefaultSkin()/createListboxDrawable()有使用，其他未使用
 * 
 * @author robins
 * 
 */
public class NinePatchUtils {
	...
	public static Drawable decodeDrawableFromAsset(Context context,
		...
		if (bitmap.getNinePatchChunk() == null) {
			...
			return new NinePatchDrawable(context.getResources(), bitmap,
					bitmap.getNinePatchChunk(), paddingRect, null);
		}
	}
	 ...
	 * @throws Exception
	 */
	public static Bitmap readFromAsset(Context context, String ninePatchPngPath)
			throws Exception {
	...
	// public static Drawable createListboxDrawable(Context context, String
	// path)
	// {
	// try{
	// NinePatchDrawable d;
	...
	// Bitmap bm =
	// SmttResource.decodeResource(SmttResource.loadIdentifierResource(path,
	// SmttResource.TYPE_DRAWABLE), opt);
	// if (bm == null)
	// {
	// return null;
	// }
	...
	// d = new NinePatchDrawable(context.getResources(), bm,
	// bm.getNinePatchChunk(), paddingRect, null);
	...
	// }
	/*
	 * Only Use for SDK 页面滚动条 读取asset目录下 skin/bmp/lsjd的滚动条图片 传入参数完整文件名
	 * 
	 * *从ui工程获得滚动条图片，图片在asset目录下 SDK从浏览器的asset目录下获取滚动条图片 robins
	 */
	public static Drawable getDrawableFromDefaultSkin(Context ctx,
			String fileName) {
	...
	// 获取web图片
	public static Drawable getWepbFromDefaultSkin(Context context,
			String fileName) {
		String finalFileName = fileName;
		if (!TextUtils.isEmpty(finalFileName)) {
			// 构建完整路径
			...
			Bitmap b = webpToBitmap(data, len, Bitmap.Config.ARGB_8888,
					getPictureScale(context), context);
	...
	public static boolean isWebP(byte[] data) {
		...
		/*
		 * Find "VP8" older is "VP8 " newer is "VP8X" or "VP8L"
		 */
	 ...
	 * @return
	 */
	public static Bitmap webpToBitmap(byte[] buffer, int len,
			Bitmap.Config config, float scale, Context context) {
		if (!isWebP(buffer)) {
			return null;
		}
		...
		Bitmap b = null;
		try {
			int[] int_pixels = libwebp.getInstance(context).decodeBase(buffer,
					width, height);
			b = Bitmap.createBitmap(int_pixels, width[0], height[0],
	...
	public static BitmapDrawable getBmpDrawable(Context ctx, Bitmap bitmap) {
		...
		try {
			...
			// arrayOfObject[0] =
			// SmttResource.getResources();//用QQ浏览器Resource还不行
